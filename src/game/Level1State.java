...
/**
 * Level1State class is a GameState class for the first level of the game.
 * @author Ian Tang
 *
 */
public class Level1State extends GameState{
	...
	public void init() {
		...
		map = new Map("/Maps/map1.map"); //initializing new map
	...
	public void tick() {	
		...
		if(xOffset > 12000) { //whenever the player reaches past the end of the level
			Player.setLevel(2); //the background is set to the second level's
			gm.states.push(new Level2State(gm)); //game manager pushes the 2nd level's state, switching the map
		}
	}
